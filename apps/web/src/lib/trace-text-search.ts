...
export function sortTracesByPreferredJournal<
  T extends { journal_id: string; date?: string | null },
>(rows: T[], preferredJournalId: string | null): T[] {
  return [...rows].sort((a, b) => {
    const ap =
      preferredJournalId && a.journal_id === preferredJournalId ? 0 : 1;
    const bp =
      preferredJournalId && b.journal_id === preferredJournalId ? 0 : 1;
