...
export const MAP_TOOLBAR_ICON_CELL = "flex size-9 shrink-0 items-center justify-center";

export const MAP_TOOLBAR_LABEL_CELL = "flex min-w-0 flex-1 items-center pr-2 text-sm font-medium text-foreground";
...
export function MapToolbarIconButton({ icon, label, active, onClick, title, className }: MapToolbarIconButtonProps) {
  return (
    <button
      type="button"
      title={title ?? label}
      onClick={onClick}
      className={cn(MAP_TOOLBAR_TRIGGER_CLASS, active && "bg-primary/10 ring-1 ring-inset ring-primary/20 hover:bg-primary/12", className)}
    >
